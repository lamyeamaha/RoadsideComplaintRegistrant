...
public class ComplaintTable extends SQLiteOpenHelper {

    // TABLE INFORMATTION
    static final String DB_NAME = "KEY_VALUE.DB";
    public final String TABLE_COMPLAINT_VALUE = "complaint_value_pairs";
    public final String COMPLAINT_KEY = "complaintname";
    public final String COMPLAINT_VALUE = "complaintvalue";
    //
    public ComplaintTable(Context context) {
    ...
    public void onCreate(SQLiteDatabase db) {    /// called only for the 1st time when the app is installed
        ...
        try {
            db.execSQL("DROP TABLE " + TABLE_COMPLAINT_VALUE);
        } catch (Exception e) {
        }
        createComplaintTable(db);
    }

    private void createComplaintTable(SQLiteDatabase db){
        try {
            db.execSQL("create table " + TABLE_COMPLAINT_VALUE + " (" + COMPLAINT_KEY
    ...
    private void handleError(SQLiteDatabase db, Exception e){
        ...
        if (errorMsg.contains("no such table")){
            if (errorMsg.contains(TABLE_COMPLAINT_VALUE)){
                createComplaintTable(db);
            }
        }
    }
    ...
    public Boolean insertKeyValue(String key, String value) {
        ...
        long result;
        try{
            result = db.insert(TABLE_COMPLAINT_VALUE, null, cv);
        }catch (Exception e){
            handleError(db, e);
            result = db.insert(TABLE_COMPLAINT_VALUE, null, cv);
        }
...
//	}
    ...
    public boolean updateValueByComplaintKey(String key, String value) {
        ...
        int nr = 0;
        try{
            nr = db.update(TABLE_COMPLAINT_VALUE, cv, COMPLAINT_KEY + "=?",
                    new String[] { key });
        }catch (Exception e){
            handleError(db, e);
            try {
    ...
    public Integer deleteDataByComplaintKey(String key) {
        ...
        int isDeleted = 0;
        try{
            isDeleted = db.delete(TABLE_COMPLAINT_VALUE, COMPLAINT_KEY + " = ?", new String[] { key });
        }catch (Exception e){
            handleError(db, e);
            try {
                isDeleted = db.delete(TABLE_COMPLAINT_VALUE, COMPLAINT_KEY + " = ?", new String[]{key});
    ...
    public String getValueByComplaintKey(String key) {
        ...
        Cursor res;
        try{
            res = db.rawQuery("SELECT * FROM " + TABLE_COMPLAINT_VALUE + " WHERE "
